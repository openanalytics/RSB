 ...
 * <b>This processor bypasses the messaging infrastructure so it is not a fair player compared to the other REST/SOAP
 * entry points, thus it should be used with care!</b>
 * 
 * @author "OpenAnalytics &lt;rsb.development@openanalytics.eu&gt;"
 */
@Component("processResource")
@Path("/" + Constants.PROCESS_PATH)
public class ProcessResource extends AbstractComponent {
    private interface FunctionCallJobBuilder {
        AbstractFunctionCallJob build(final String applicationName, final UUID jobId, final GregorianCalendar submissionTime)
                throws IOException;
    }
    ...
    public Response processJsonFunctionCallJob(final String jsonArgument, @Context final HttpHeaders httpHeaders,
            @Context final UriInfo uriInfo) throws Exception {
        return handleNewRestJob(httpHeaders, uriInfo, new FunctionCallJobBuilder() {
            public AbstractFunctionCallJob build(final String applicationName, final UUID jobId, final GregorianCalendar submissionTime) {
                return new JsonFunctionCallJob(Source.REST_IMMEDIATE, applicationName, jobId, submissionTime, jsonArgument);
            }
        });
    }
    ...
    public Response processXmlFunctionCallJob(final String xmlArgument, @Context final HttpHeaders httpHeaders,
        ...
        return handleNewRestJob(httpHeaders, uriInfo, new FunctionCallJobBuilder() {
            public AbstractFunctionCallJob build(final String applicationName, final UUID jobId, final GregorianCalendar submissionTime) {
                return new XmlFunctionCallJob(Source.REST_IMMEDIATE, applicationName, jobId, submissionTime, xmlArgument);
            }
        });
    }

    private Response handleNewRestJob(final HttpHeaders httpHeaders, final UriInfo uriInfo, final FunctionCallJobBuilder jobBuilder)
            throws Exception {
    ...
    private Response handleNewJob(final String applicationName, final HttpHeaders httpHeaders, final UriInfo uriInfo,
        ...
        final AbstractFunctionCallJob job = jobBuilder.build(applicationName, jobId, (GregorianCalendar) GregorianCalendar.getInstance());
