...
public class JmsMessageDispatcher extends AbstractComponent implements MessageDispatcher {
	...
	static final class WorkItemMessagePostProcessor implements MessagePostProcessor {
		...
		public Message postProcessMessage(final Message message) throws JMSException {
			message.setStringProperty(Constants.SOURCE_MESSAGE_HEADER,
					this.workItem.getSource().toString() );
			message.setStringProperty(Constants.APPLICATION_NAME_MESSAGE_HEADER,
					this.workItem.getApplicationName() );
			message.setStringProperty(Constants.JOB_ID_MESSAGE_HEADER,
					this.workItem.getJobId().toString() );
			message.setJMSPriority(this.workItem.getPriority());
			return message;
		}
	...
	public void dispatch(final AbstractJob job) {
		this.jmsTemplate.convertAndSend(getJobQueueName(job), job,
				new WorkItemMessagePostProcessor(job) );
	}
	
	@Override
	public void dispatch(final AbstractResult<?> result) {
		this.jmsTemplate.convertAndSend(getResultQueueName(result), result,
				new WorkItemMessagePostProcessor(result) );
	}
	...
	public <T extends AbstractResult<?>> T process(final AbstractJob job) {
		dispatch(job);
		final Object result= this.jmsTemplate.receiveSelectedAndConvert(getResultQueueName(job),
